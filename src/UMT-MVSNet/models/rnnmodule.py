...
class Transformer_FeatNet(nn.Module):
    ...
    def forward(self, x):  # label is unused if self.cos_layer == 'no'
        ...
        feat = self.bottleneck(global_feat[0])
        return feat
...
class SemanticNet(nn.Module): 
    def __init__(self, with_semantic_map=False):
        super(SemanticNet, self).__init__()

        self.with_semantic_map = with_semantic_map
        if with_semantic_map:
            self.conv_before = ConvGnReLU(4, 8, stride=2)
        else:
            self.conv_before = ConvGnReLU(3, 8, stride=2)

        batchNorm_momentum = 0.1

        self.conv11 = nn.Conv2d(8, 64, kernel_size=3, padding=1)
    ...
    def forward(self, x, semantic_map=None):
        ...
        if self.with_semantic_map:
            x = self.conv_before(torch.cat([x, semantic_map], dim=1))
        else:
